...
public enum InternalJvm {
    ...
    private static final boolean IS_DEBUG = INPUT_ARGUMENTS2.contains("jdwp") || InternalJvm.getBoolean("debug");
    ...
    private static final boolean IS_FLIGHT_RECORDER = INPUT_ARGUMENTS2.contains(" -XX:+FlightRecorder") || InternalJvm.getBoolean("jfr");
    ...
    private static final boolean IS_ARM = InternalJvm.getBoolean("jvm.isarm") ||
    ...
    static {
        ...
        try {
            ...
            System.err.println(InternalJvm.class.getName() + ": Unable to determine the reservedMemory value, will always report 0");
        ...
        SAFEPOINT_ENABLED = InternalJvm.getBoolean("jvm.safepoint.enabled");

        RESOURCE_TRACING = InternalJvm.getBoolean("jvm.resource.tracing");

        Logger logger = LoggerFactory.getLogger(InternalJvm.class);
        logger.info("Chronicle core loaded from " + InternalJvm.class.getProtectionDomain().getCodeSource().getLocation());
        if (RESOURCE_TRACING)
    ...
    private static void loadSystemProperties(String name, boolean wasSet) {
        try {
            ClassLoader classLoader = InternalJvm.class.getClassLoader();
            ...
            try (InputStream is = is0) {
                if (is == null) {
                    (wasSet ? Slf4jExceptionHandler.WARN : Slf4jExceptionHandler.DEBUG)
                            .on(InternalJvm.class, "No " + name + " file found");

                } else {
                    Properties prop = new Properties();
                    prop.load(is);
                    System.getProperties().putAll(prop);
                    Slf4jExceptionHandler.DEBUG.on(InternalJvm.class, "Loaded " + name + " with " + prop);
                }
            }
        } catch (Exception e) {
            Slf4jExceptionHandler.WARN.on(InternalJvm.class, "Error loading " + name, e);
        }
    }
    ...
    private static int getProcessId0() {
        ...
        if (pid == null) {
            ...
            System.err.println(InternalJvm.class.getName() + ": Unable to determine PID, picked a random number=" + rpid);
            return rpid;
        } else {
    ...
    public static void setAccessible(AccessibleObject h) {
        if (IS_JAVA_9_PLUS)
            try {
                ...
                throw InternalJvm.rethrow(throwable);
            }
        else
    ...
    private static long maxDirectMemory0() {
        ...
        System.err.println(InternalJvm.class.getName() + ": Unable to determine max direct memory");
        return 0L;
    }
    ...
    public static void signalHandler(SignalHandler signalHandler) {
        if (signalHandlerGlobal.handlers.isEmpty()) {
            if (!net.openhft.chronicle.core.OS.isWindows()) // not available on windows.
        ...
        SignalHandler signalHandler2 = signal -> {
            InternalJvm.warn().on(signalHandler.getClass(), "Signal " + signal.getName() + " triggered");
    ...
    private static void addSignalHandler(String sig, SignalHandler signalHandler) {
        try {
            ...
            // System.exit()
            InternalJvm.warn().on(signalHandler.getClass(), "Unable add a signal handler", e);
        }
    }
    ...
    public static ClassMetrics classMetrics(Class c) throws IllegalArgumentException {
        return CLASS_METRICS_MAP.computeIfAbsent(c, InternalJvm::getClassMetrics);
    }
        ...
        for (URL url : ucl.getURLs()) {
            try {
                ...
                if (!jcp.contains(path)) {
                    if (isDebugEnabled(InternalJvm.class))
                        debug().on(InternalJvm.class, "Adding " + path + " to the classpath");
                    classpath.append(File.pathSeparator).append(path);
                }
            } catch (Throwable e) {
                debug().on(InternalJvm.class, "Could not add URL " + url + " to classpath");
            }
        }
    ...
    public static double getDouble(String property, double defaultValue) {
        ...
        if (value != null)
            try {
                ...
                InternalJvm.debug().on(InternalJvm.class, "Unable to parse property " + property + " as a double " + e);
            }
        return defaultValue;
    }
    ...
    static class CommonInterruptible {
        ...
        public void interrupt() {
            ...
            if (InternalJvm.isDebugEnabled(getClass()))
                InternalJvm.debug().on(clazz, fc + " not closed on interrupt");
            inside.set(false);
        }
    }
        ...
        public void handle(Signal signal) {
            for (SignalHandler handler : handlers) {
                try {
                    ...
                    InternalJvm.warn().on(this.getClass(), "Problem handling signal", t);
                }
            }
        }
    }
